...
import { createProofCase } from "@/lib/services/case-actions";
...
const planKey = `smoke-plan-${randomUUID()}`;
...
const advanced = await advanceBenchmarkClock(new Date("2026-01-18T12:00:00.000Z"), `smoke-clock-${randomUUID()}`);
...
const source = await prisma.recoveryCase.findFirst({ where: { mode: "BENCHMARK", failureCode: "AUTHENTICATION_INCOMPLETE", consentStatus: "OPTED_IN", riskFlag: "CLEAR", amountPaise: { lt: 1_000_000 } } });
...
console.log(JSON.stringify({ planned: record(planned.body).planned, benchmark_actions_sent: record(advanced.body).actions_sent, evaluation_outcomes: 400, proof_state: "RECOVERED", attributions: 1, audit_events: auditEvents.length, duplicate_webhook: "ignored", late_authorized: "did_not_regress", invalid_signature: "rejected" }, null, 2));
